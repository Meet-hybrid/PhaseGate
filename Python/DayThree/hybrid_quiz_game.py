def main():
		...
		| football knowledge.                                                |
		| Make the right selections and win a ticket to the WORLD CUP!       |
		======================================================================
		""")

		name = input("Enter you name to test your football knowledge: ")
		print(f"\nHello {name.upper()}! get ready to begin")
		input("Press ENTER to start the game\n")
		play_game()
	
quiz = [
    ("Who's the best footballer in the history of football?", "Mane", "Mikel obi", "CR7", "Bale", "C"),
    ("Which club is known as 'THE RED DEVILS'?", "Manchester United", "Chelsea", "Juventus", "Club with no European trophy", "A"),
...
def play_game():
	...
	while number < len(quiz):
		...
		if answer == correct:
			...
			score += 1
			if score == 10:
				print("See you in the world cup")
				
		elif answer in options:
			correct_text = {"A": a, "B": b, "C": c, "D": d}[correct]
			print(f"Wrong! Correct answer was {correct}. {correct_text}")
		else:
			print(" Olodo! Invalid input. Please enter A, B, C, or D. now you have to start afresh")
			play_game()	
